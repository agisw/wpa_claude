...
// const url = "https://dsds.mwebdev.samsungds.net/storybooks-dev/vue-vuetify/";
// const outputDir = "screenshots"

const takeScreenshot = async (url, outputDir = "screenshots") => {
  try {
    ...
    // Storybook specific tuning
    if (url.includes("/storybook")) {
      ...
      await driver.wait(until.elementLocated(By.css('[aria-label="Enter full screen"]')), 1000)

      // Click the "Go full screen" button if it exists
      try {
        const fullscreenBtn = await driver.findElement(By.css('[aria-label="Enter full screen"]'))
      ...
      // 3. 폰트 로딩 완료 대기
      await driver.wait(
        () => driver.executeScript('return document.fonts.ready'),
        1000
      );
    }
    ...
    // driver path: https://storage.googleapis.com/chrome-for-testing-public/139.0.7258.154/linux64/chromedriver-linux64.zip
    return null
  }
