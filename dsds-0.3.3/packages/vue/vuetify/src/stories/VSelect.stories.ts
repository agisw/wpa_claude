...
import { radioControl, replaceTemplateContent } from "./utils"
...
Vuetify의 VSelect를 DSDS 디자인 시스템 스타일로 래핑한 셀렉트 컴포넌트입니다.
...
### 디자인 일관성 정책 <policy />
DSDS 디자인 시스템의 일관성을 위해 다음과 같은 Vuetify VSelect의 기능들을 **의도적으로 지원하지 않습니다**:
...
\`\`\`html
<template>
  <VSelect v-model="selectedValues"
           :items="items"
           multiple
...
\`\`\`html
<template>
  <VSelect v-model="selectedValue" :items="items">
    <template #selection="{ item }">
      <span class="custom-selection">{{ item.raw.shortTitle }}</span>
    </template>
  </VSelect>
</template>
...
const selectedValue = ref('')
const items = ref([
  { title: '긴 제목 1', value: '1', shortTitle: '짧은1' },
  { title: '긴 제목 2', value: '2', shortTitle: '짧은2' }
])
</script>
\`\`\`
...
- \`autocomplete\` (boolean, optional): 자동완성 설정

### v-model
VSelect은 Vue 3의 defineModel을 사용하여 양방향 바인딩을 지원합니다:
...
<!-- 다중 선택 -->
<VSelect v-model="selectedValues" :items="items" multiple />
\`\`\`
...
### 접근성 <accessibility />
- 키보드 네비게이션 지원 (Tab, Arrow keys, Enter, Space)
...
const sampleItems = ["selected", "selected1", "selected2", "selected3", "selected4"].map((item) => ({
  title: item,
...
const VSELECT_BASE_TEMPLATE = '<VSelect v-bind="args" v-model="selected" />'

const VSELECT_SIZES_TEMPLATE = `
  <div class="flex flex-col gap-3">
    <VSelect v-bind="args" width="120" v-model="largeSel" size="large" placeholder="Large" />
    <VSelect v-bind="args" width="120" v-model="mediumSel" size="medium" placeholder="Medium" />
    <VSelect v-bind="args" width="108" v-model="smallSel" size="small" placeholder="Small" />
  </div>
`
...
export const Default: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "비활성화된 Select 예제입니다.",
      },
      source: {
        code: VSELECT_BASE_TEMPLATE,
      },
    },
  },
  render: (args) => ({
    ...
    template: VSELECT_BASE_TEMPLATE,
  }),
}
...
const customSelectionSlotTPL = `
  <VSelect v-bind="args" v-model="selected">
    <template #selection="{ item }">
      <span>{{ item.raw.shortTitle }}</span>
    </template>
  </VSelect>
`
/**
 * 기본 Select 의 Selection 슬롯을 커스텀하는 예제입니다.
 */
export const CustomSelectionSlot: Story = {
    ...
    docs: {
      ...
      source: {
        transform: (code: string) => {
          ...
          // </template> : 종료 태그
          return replaceTemplateContent(code, customSelectionSlotTPL)
        },
      },
    },
  },
  render: (args) => ({
    ...
    template: customSelectionSlotTPL,
  }),
}
...
Sizes.render = (args) => ({
  ...
  template: VSELECT_SIZES_TEMPLATE,
})
