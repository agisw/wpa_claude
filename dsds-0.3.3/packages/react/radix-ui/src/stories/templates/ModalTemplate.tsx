...
export type ModalTemplateProps = Pick<ModalProps, "title" | "size" | "content" | "className"> &
...
export function ModalTemplate({ isMessage, title, size, content, message, className }: ModalTemplateProps) {
  return (
    <div>
      {isMessage ? (
        <MessageModal defaultOpen message={message} trigger={trigger} footer={footerMessage} className={className} />
      ) : (
        <Modal
          defaultOpen
          title={title}
