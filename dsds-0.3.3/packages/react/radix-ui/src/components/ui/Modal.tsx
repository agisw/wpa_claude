...
function ModalOverlay({ className, ...props }: React.ComponentProps<typeof ModalPrimitive.Overlay>) {
  return (
    <ModalPrimitive.Overlay
      ...
      className={cn(
        "z-overlay bg-modal-backdrop fixed inset-0",
...
function ModalFrame({
  ...
  return (
    <ModalPortal data-slot="dialog-portal">
      {className && className.includes("noOverlay") ? null : <ModalOverlay />}
      <ModalPrimitive.Content
        onInteractOutside={(e) => e.preventDefault()}
        data-slot="dialog-content"
        className={cn(
          // Base
          "bg-background z-modal fixed flex flex-col rounded-[6px] text-[var(--colors-neutral-neutral-17)] shadow-lg duration-100",
          // Padding (Optional)
          // "max-h-[calc(100vh-40px)] max-w-[calc(100vw-40px)] overflow-hidden",
          // Spacing
        ...
        {...props}
      >
        <div className={cn("flex flex-row items-center justify-between px-[20px] pt-[16px] pb-[16px]")}>
          <ModalTitle className="typo-dialog-header h-[20px]">{title}</ModalTitle>
          <ModalPrimitive.Close data-slot="dialog-close" className="mr-[-4px] flex flex-row justify-end">
            <button
              className={cn(
        ...
        {title && <div className="h-[0.09px] shrink-0 bg-[var(--color-border-border-3-inner)]" />}
        {children}
      </ModalPrimitive.Content>
    </ModalPortal>
  )
...
function ModalContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col overflow-y-auto px-[20px] text-left", className)}
      {...props}
    />
  )
}
...
function ModalFooter({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="dialog-footer" className={cn("flex shrink-0 flex-col px-[20px]", className)} {...props} />
}
...
function ModalDescription({ className, ...props }: React.ComponentProps<typeof ModalPrimitive.Description>) {
  return (
    <ModalPrimitive.Description
      ...
      className={cn("flex min-h-0 overflow-y-auto whitespace-pre-line", className)}
      {...props}
    />
  )
}
...
export function Modal({ title, size, content, trigger, defaultOpen, footer, className }: ModalProps) {
  return (
    <ModalBase defaultOpen={defaultOpen}>
      <ModalTrigger>{trigger}</ModalTrigger>
      <ModalFrame title={title} className={cn(ModalWidth[size], className)}>
        <ModalContent>
          <ModalDescription className={cn("mt-[12px] mb-[8px]", ModalDescriptionHeight[size])}>
            {content}
          </ModalDescription>
        </ModalContent>
        <div className="h-[1px] shrink-0 bg-[var(--color-border-border-3-inner)]" />
...
export function MessageModal({ message, trigger, defaultOpen, footer, className }: MessageModalProps) {
  return (
    <ModalBase defaultOpen={defaultOpen}>
      <ModalTrigger>{trigger}</ModalTrigger>
      <ModalFrame className={cn("max-h-[340px] w-[340px]", className)}>
        {/* 반응형일 경우 기본 높이 적용 */}
        <ModalContent className={cn(className && className?.includes("overflow-hidden") ? "h-[96px]" : "min-h-[96px]")}>
          <ModalDescription className="my-[8px]">{message}</ModalDescription>
        </ModalContent>
        <ModalFooter className="shrink-0">{footer}</ModalFooter>
      </ModalFrame>
    </ModalBase>
  )
}
...
export function ModalCheckbox({
  ...
  return (
    <div className="flex items-center">
      ...
      {label && (
        <Label htmlFor={id} className="mr-[16px]">
          {label}
        </Label>
      )}
    </div>
