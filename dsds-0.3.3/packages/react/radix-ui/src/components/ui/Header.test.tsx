...
const { Default, WithGNBAndUtility, CustomLayout_NoVisualRegression, CustomLayoutWithGNBUtility } =
...
describe("Header Tests with Storybook", () => {
  test("초기 렌더링 시 모든 요소가 올바르게 표시되어야 한다.", () => {
    ...
    expect(headerElement).toHaveClass("dsds-header", "dsds-header-light", "dsds-header-compact")
  ...
  test("CustomLayout 스토리가 올바르게 표시되어야 한다.", () => {
    render(<CustomLayout_NoVisualRegression />)
  ...
  describe("GNB와 Utility 기능 테스트", () => {
    test("gnb prop으로 전달된 아이템들이 올바르게 렌더링되어야 한다.", () => {
      ...
      // GNB 컨테이너가 존재하는지 확인
      const gnbContainer = screen.getByTestId("dsds-header-gnb")
      expect(gnbContainer).toBeInTheDocument()
    ...
    test("utility prop으로 전달된 아이템들이 올바르게 렌더링되어야 한다.", () => {
      ...
      const utilityContainer = screen.getByTestId("dsds-header-utility")
      ...
      // 햄버거 메뉴가 렌더링되는지 확인
      const hamburgerMenu = screen.getByTestId("dsds-header-hamburger")
    ...
    test("커스텀 레이아웃에서 Header.GNB와 Header.Utility 컨테이너가 올바르게 동작해야 한다.", () => {
      ...
      // 컨테이너 내부 요소들 확인
      const docsMenu = screen.getByRole("button", { name: /Tenant/i })
      expect(docsMenu).toBeInTheDocument()
      const homeMenu = screen.getByRole("button", { name: /Menu/i })
    ...
    test("Header.HamburgerMenu 컴포넌트가 올바르게 렌더링되어야 한다.", () => {
      ...
      // 햄버거 메뉴 버튼이 존재하는지 확인
      const hamburgerButton = screen.getByTestId("dsds-header-hamburger")
    ...
    test("HeaderItem에 tenant와 hamburger 타입이 올바르게 처리되어야 한다.", () => {
      ...
      const tenantButton = screen.getByRole("button", { name: /Tenant/i })
      expect(tenantButton).toBeInTheDocument()
      expect(tenantButton).toHaveClass("dsds-header-tenant")

      // hamburger 타입 버튼 확인
      const hamburgerButton = screen.getByTestId("dsds-header-hamburger")
      expect(hamburgerButton).toBeInTheDocument()
      expect(hamburgerButton).toHaveClass("dsds-header-hamburger")
    })
  })
})
