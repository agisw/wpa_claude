...
const boxVariantsConfig: TextboxVariantsConfig = {
  variant: {
    default: "dsds-text-box-default",
    ghost: "dsds-text-box-ghost",
  },
  size: {
    small: "text-box-small",
    medium: "text-box-medium",
    large: "text-box-large",
  },
}
...
const disabledClassNames = {
  default: "disabled dsds-text-box-default-disabled",
  ghost: "disabled dsds-text-box-ghost-disabled",
}
...
const TextboxWrapper = forwardRef<HTMLTextAreaElement | HTMLInputElement | HTMLButtonElement, TextboxWrapperProps>(
  (
    ...
    const [isPopulated, setIsPopulated] = useState(!!value)

    // value prop이 변경될 때 populated 상태 업데이트 (controlled 모드 지원)
    useEffect(() => {
    ...
    const handleChange: React.EventHandler<React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>> = (e) => {
      ...
      // controlled 또는 uncontrolled 모드에서 값이 있는지 체크하여 populated 상태 업데이트
    ...
    const populatedClassName = isPopulated ? populatedClassNames[variant!] : null
    console.log({ isPopulated, populatedClassName })
    ...
    return (
      <div className={cn("text-box-wrapper", className)} data-testid={dataTestId}>
        <Basicbox
        ...
        >
          {
            ...
            !iconOnly && multiline ? (
              <textarea {...textAreaProps} disabled={disabled} rows={3} required={required} id={id} ref={textAreaRef} />
            ) : asButton ? (
              <button
                id={id}
                disabled={disabled}
              ...
              <input id={id} disabled={disabled} {...inputProps} type={type} required={required} />
            )
          }
        </Basicbox>
      </div>
